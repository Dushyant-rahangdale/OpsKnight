...
function SubmitButton({ disabled }: { disabled?: boolean }) {
  ...
  return (
    <button
      ...
      style={{
        background: disabled ? '#e5e7eb' : 'linear-gradient(135deg, #1e293b 0%, #0f172a 100%)',
...
export default function UserCreateForm({ action, className = '', disabled = false }: Props) {
  ...
  return (
    <form ref={formRef} action={formAction} className={`invite-form ${className}`.trim()}>
      <fieldset
        disabled={disabled}
        style={{ border: 'none', padding: 0, margin: 0, opacity: disabled ? 0.6 : 1 }}
      >
      ...
      {state?.error && <div className="form-error">{getUserFriendlyError(state.error)}</div>}
      {state?.inviteUrl && (
        <div className="invite-link-feedback">
          <div className="invite-link-label">Share this invite link with the new user:</div>
          <div className="invite-link-copy">
            <input value={state.inviteUrl} readOnly className="form-input invite-link-input" />
            <button
              type="button"
